...
public interface APIClient {
  ...
  /**
   * 根据搜索条件查询DNS记录，并返回指定页码的查询结果。注意方法最多获取前10000条查询结果。 每调用一次该方法，当前API User的API请求次数减1。
   *
   * @param query 查询条件
   * @param page 页码
   * @param pageSize 页大小
  ...
  /**
   * 根据搜索条件创建一个Scan DNS任务。 每调用一次该方法，当前API User的API请求次数减1。
   *
   * @param query 查询条件
  ...
  /**
   * 获取指定scan ID的下一次scan结果。 每调用一次该方法，当前API User的API请求次数减1。
   *
  ...
  /**
   * 根据查询条件，创建一个可以迭代的自动查询DNS的{@link ScanResult}对象。 每调用一次该方法，当前API User的API请求次数减1。
   *
   * @param query 查询条件
